...
def automate_randoop(project_dir, test_class, output_limit=500):
    ...
    randoop_cmd = (f"java -classpath {project_dir}/target/classes/:{project_dir}/randoop-lib/randoop-all-4.2.1.jar "
                  f"randoop.main.Main gentests --testclass={test_class} --output-limit={output_limit}")
    run_command(randoop_cmd, project_dir)

    # Step 3: Clean temporary files
    print("\nStep 3: Cleaning temporary files...")
    run_command("rm *.class", project_dir)
